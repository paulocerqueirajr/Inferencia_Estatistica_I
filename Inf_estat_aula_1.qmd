...
plot(n, p, type="l", 
     ylab = "Proporção Amostral", 
     xlab="n")
abline(h=0.5, col="red")
legend("topright", c("Real", "Estimado"), 
...
::: {.callout-note}
# Código de geração da figura:

```{r, eval=FALSE}


library(plotly); library(ggplot2)
n <- c(seq(1,100, by=19), seq(200,1000, by=100)) 
n.sim <- 100;med <- c(); n.size <- c() 
for(i in 1:length(n)){
  for(j in 1:n.sim){
    med    <- c(med, mean(rnorm(n[i],mean = 10, 2)))
    n.size <- c(n.size, n[i]) 
...
dados <- data.frame(med, n.size)
p <- ggplot(data=dados, aes(x = factor(n.size), y = med)) + geom_point() + 
  geom_hline(yintercept = 10, color="red") + xlab("Tamanho amostral") + 
  ylab("Média amostral") + labs(title = "Exemplo TCL - X ~ N(10,4)")
ggplotly(p)
